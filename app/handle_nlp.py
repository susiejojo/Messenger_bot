...
def firstEntity(nlp, name):
    if nlp.get("entities") and nlp["entities"].get(name):
...
def handle_hate_and_confi(message):
    intent_response = client_intent.message(message["text"])
    phone_number = firstEntity(message["nlp"], 'wit$phone_number:phone_number')
    location = firstEntity(message["nlp"], 'wit$location:location')
    email = firstEntity(message["nlp"], 'wit$email:email')
    if intent_response.get('intents') and intent_response["intents"][0]["name"] == 'hatespeech' and intent_response["intents"][0]["confidence"]>0.7:
        print(intent_response["intents"][0]["confidence"])
        return 1
    elif phone_number and phone_number["confidence"]>0.8:
        return 2
    elif location and location["confidence"]>0.8:
        return 2
    elif email and email["confidence"]>0.8:
        return 2
    else:
        return 0
